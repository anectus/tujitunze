...
export default function BankProfilePage() {
  ...
  const t = bankProfileTranslations[language];
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(data.message || t.unableToLoadProfile);
        }
        ...
        setError(err instanceof Error ? err.message : t.unableToLoadProfile);
      } finally {
        setLoading(false);
      }
    };
  ...
  const saveContact = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || t.unableToUpdateContact);
      }

      setProfile(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : t.unableToUpdateContact);
    } finally {
  ...
  const regenerateApiKey = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || t.unableToRegenerateKey);
      }
      ...
      setError(err instanceof Error ? err.message : t.unableToRegenerateKey);
    }
  };
  ...
  const saveWebhook = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || t.unableToConfigureWebhook);
      }
      ...
      setError(err instanceof Error ? err.message : t.unableToConfigureWebhook);
    } finally {
      ...
      <div className="p-4 sm:p-8">
        ...
        {revealedSecret && (
          <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
            <p className="font-semibold">{revealedSecret.label} — {t.shownOnceNote}</p>
          ...
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
              ...
              <dl className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <dt className="text-xs font-semibold uppercase text-gray-500">{t.bankCode}</dt>
                  <dd className="mt-1 text-gray-900">{profile.bankCode || "—"}</dd>
                </div>

                <div>
                  <dt className="text-xs font-semibold uppercase text-gray-500">{t.swiftCode}</dt>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
              ...
              <p className="text-lg font-bold text-gray-900">{t.branchInformationTitle}</p>

              {profile.branches.length === 0 ? (
                <p className="mt-2 text-sm text-gray-500">{t.noBranches}</p>
              ) : (
                  ...
                  {profile.branches.map((branch) => (
                    <li key={branch.branch_id} className="py-3">
                      ...
                      <p className="text-sm text-gray-500">
                        {branch.branch_code} · {[branch.district, branch.region].filter(Boolean).join(", ") || t.locationNotOnFile}
                      </p>
                    </li>
                  ))}
                </ul>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
              ...
              <p className="text-lg font-bold text-gray-900">{t.contactInformationTitle}</p>
              ...
              <form onSubmit={saveContact} className="mt-4 grid gap-4 sm:grid-cols-2">
                ...
                <div className="sm:col-span-2">
                  ...
                  >
                    {savingContact ? t.saving : t.saveContactInformation}
                  </button>
                </div>
            ...
            <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">
              ...
              <p className="text-lg font-bold text-gray-900">{t.integrationStatusTitle}</p>

              <div className="mt-4 flex items-center justify-between rounded-lg bg-gray-50 px-4 py-3">
                <span className="text-sm text-gray-700">
                  {profile.apiKey.hasKey
                    ? `${t.activeApiKeyPrefix} •••• ${profile.apiKey.preview}`
                    : t.noApiKeyYet}
                </span>
              ...
              <div className="mt-6 border-t border-gray-100 pt-4">
                <p className="text-sm font-semibold text-gray-700">{t.connectionTestingTitle}</p>
            ...
            <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-6 text-center">
              <p className="text-gray-600">{t.securitySettingsTitle}</p>
              <p className="mt-2 text-sm text-gray-400">
                {t.securitySettingsNote}
              </p>
            </div>

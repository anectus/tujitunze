...
// POST /telecom/reconciliation/runs returns camelCase (matches the rest
// of this service's write-endpoint convention), unlike the snake_case
// raw-row shape the GET list endpoint above returns — these are
// deliberately two different interfaces, not a typo.
...
const SAMPLE_PLACEHOLDER =
  "reference,amount,date\nTXN-1001,5000,2026-08-17\nTXN-1002,12000,2026-08-17";

export default function ReconciliationPage() {
  const router = useRouter();
  const { language } = useLanguage();
  const t = telecomReconciliationTranslations[language];
  ...
  const handleUpload = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || t.runError);
      }
      ...
      setUploadError(err instanceof Error ? err.message : t.runError);
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <div className="p-4 sm:p-8">
        ...
        {/* Upload: Telecom Records vs HSIMS Records */}
        <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-md">

          <p className="text-lg font-bold text-gray-900">{t.uploadHeading}</p>
          ...
          <form onSubmit={handleUpload} className="mt-4">
            ...
            >
              {uploading ? t.reconciling : t.runReconciliation}
            </button>
        ...
        <div className="mt-6">
          ...
          <p className="mb-2 text-sm font-semibold text-gray-700">{t.historyHeading}</p>
            ...
            <div className="overflow-x-auto rounded-2xl border border-gray-100 bg-white shadow-md">
              ...
              <table className="w-full text-left text-sm">
                ...
                <thead className="bg-gray-50 text-xs uppercase text-gray-500">
                  <tr>
                    <th className="px-6 py-3 font-semibold">{t.run}</th>
                    <th className="px-6 py-3 font-semibold">{t.uploaded}</th>
                    <th className="px-6 py-3 font-semibold">{t.matched}</th>
                    <th className="px-6 py-3 font-semibold">{t.unmatched}</th>
                  ...
                  {runs.map((run) => (
                    <tr key={run.run_id}>
                      ...
                      <td className="px-6 py-4">
                        <Link
                          href={`/telecom/reconciliation/${run.run_id}`}

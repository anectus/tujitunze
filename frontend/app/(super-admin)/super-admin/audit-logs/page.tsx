"use client";

import { useEffect, useState } from "react";
import { useRouter } from "next/navigation";

import { getAccessToken } from "@/lib/utils/permissions";
import DashboardHeader from "@/components/dashboard/DashboardHeader";
import PageContainer from "@/components/dashboard/PageContainer";
import QuickActions, {
  type QuickAction,
} from "@/components/dashboard/QuickActions";
import { useLanguage } from "@/lib/context/LanguageContext";
import { adminAuditLogsTranslations } from "@/constants/translations/admin-audit-logs";
import { commonTranslations } from "@/constants/translations/common";
import { superAdminDashboardTranslations } from "@/constants/translations/super-admin-dashboard";
import { API_URL } from "@/lib/utils/api";
import { CoinsIcon, KeyIcon, PlusIcon } from "@/components/common/SidebarIcons";

interface AuditLogEntry {
  auditId: number;
  memberId: number | null;
  actionType: string;
  affectedTable: string | null;
  affectedRecordId: number | null;
  oldValue: Record<string, unknown> | null;
  newValue: Record<string, unknown> | null;
  ipAddress: string | null;
  createdAt: string;
}

// Reuses the same GET /admin/audit-logs endpoint the Admin audit-logs page
// reads from (the backend guard now also allows Super-admin, see
// audit-logs.controller.ts) rather than standing up a parallel module —
// there is exactly one audit_logs table and one place that reads it.
export default function SuperAdminAuditLogsPage() {
  const router = useRouter();
  const { language } = useLanguage();
  const t = adminAuditLogsTranslations[language];
  const common = commonTranslations[language];
  const dt = superAdminDashboardTranslations[language];

  const quickActions: QuickAction[] = [
    {
      label: dt.quickActionCreateAdministrator,
      href: "/super-admin/administrators",
      icon: PlusIcon,
    },
    {
      label: dt.quickActionRolesPermissions,
      href: "/super-admin/roles",
      icon: KeyIcon,
    },
    {
      label: dt.quickActionSavingRules,
      href: "/super-admin/saving-rules",
      icon: CoinsIcon,
    },
  ];

  const [logs, setLogs] = useState<AuditLogEntry[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");

  const loadLogs = async () => {
    const token = getAccessToken();

    if (!token) {
      router.push("/login");
      return;
    }

    try {
      const response = await fetch(
        `${API_URL}/admin/audit-logs`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

      if (response.status === 401 || response.status === 403) {
        router.push("/login");
        return;
      }

      const data = await response.json();

      if (!response.ok) {
        throw new Error(data.message || t.loadError);
      }

      setLogs(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : t.loadError);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- initial fetch on mount, not a derived-state sync
    loadLogs();
  // eslint-disable-next-line react-hooks/exhaustive-deps -- mount-only fetch, not a value this effect should re-sync to
  }, [router, t.loadError]);

  // Read-only list — safe to silently replace on tab focus. Also the
  // page most likely to visibly benefit: it's where a failed
  // create/update attempt elsewhere (see saving-rules.service.ts's
  // recordFailedAttempt) would show up.
  useEffect(() => {
    window.addEventListener("focus", loadLogs);
    return () => window.removeEventListener("focus", loadLogs);
    // eslint-disable-next-line react-hooks/exhaustive-deps -- subscribe once
  }, []);

  return (
    <div>

      <DashboardHeader title={t.title} />

      <PageContainer maxWidth="6xl">

        <QuickActions actions={quickActions} />

        <p className="mt-6 text-sm text-slate-600">{t.subtitle}</p>

        {error && (
          <div className="mt-6 rounded-lg bg-red-100 px-4 py-3 text-sm text-red-700">
            {error}
          </div>
        )}

        {loading ? (

          <p className="mt-8 text-slate-500">{t.loading}</p>

        ) : logs.length === 0 ? (

          <p className="mt-8 text-slate-500">{t.empty}</p>

        ) : (

          <div className="mt-8 overflow-x-auto rounded-2xl border border-gray-100 bg-white shadow-md">

            <table className="w-full text-left text-sm">

              <thead className="bg-gray-50 text-xs uppercase text-slate-500">
                <tr>
                  <th className="px-6 py-3 font-semibold">{t.colWhen}</th>
                  <th className="px-6 py-3 font-semibold">{t.colAction}</th>
                  <th className="px-6 py-3 font-semibold">{t.colTable}</th>
                  <th className="px-6 py-3 font-semibold">{t.colRecordId}</th>
                  <th className="px-6 py-3 font-semibold">{t.colActor}</th>
                  <th className="px-6 py-3 font-semibold">{t.colIpAddress}</th>
                  <th className="px-6 py-3 font-semibold">{t.colChange}</th>
                </tr>
              </thead>

              <tbody className="divide-y divide-gray-100">

                {logs.map((log) => (

                  <tr key={log.auditId} className="transition hover:bg-gray-50">

                    <td className="px-6 py-4 whitespace-nowrap text-slate-600">
                      {new Date(log.createdAt).toLocaleString("en-TZ")}
                    </td>

                    <td className="px-6 py-4 font-medium text-gray-900">
                      {log.actionType}
                    </td>

                    <td className="px-6 py-4 text-slate-600">
                      {log.affectedTable || "—"}
                    </td>

                    <td className="px-6 py-4 text-slate-600">
                      {log.affectedRecordId ?? "—"}
                    </td>

                    <td className="px-6 py-4 text-slate-600">
                      {log.memberId ?? "—"}
                    </td>

                    <td className="px-6 py-4 text-slate-600">
                      {log.ipAddress || "—"}
                    </td>

                    <td className="px-6 py-4 text-slate-600">
                      {(log.oldValue || log.newValue) && (
                        <details>
                          <summary className="cursor-pointer text-emerald-700 hover:text-emerald-800">
                            {common.view}
                          </summary>
                          <pre className="mt-2 max-w-xs whitespace-pre-wrap break-words text-xs text-slate-500">
                            {log.oldValue &&
                              `old: ${JSON.stringify(log.oldValue)}\n`}
                            {log.newValue &&
                              `new: ${JSON.stringify(log.newValue)}`}
                          </pre>
                        </details>
                      )}
                    </td>

                  </tr>

                ))}

              </tbody>

            </table>

          </div>

        )}

      </PageContainer>

    </div>
  );
}

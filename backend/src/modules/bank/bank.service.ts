...
export class BankService {
  ...
  // TUJITUNZE's own reference, independent of whatever the bank calls
  // this transaction — mirrors TelecomService.generateInternalReference.
  ...
  // Real write path for the BANK_TRANSFER contribution channel — Bank
  // staff record a member's incoming bank-transfer contribution against
  // one of the member's own linked accounts (member_bank_accounts).
  // Mirrors TelecomService.recordContribution()'s shape exactly so both
  // channels feed the same wallet ledger the same way.
  // transaction_reference's UNIQUE constraint is the idempotency guard.
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        if (!account) {
          throw new NotFoundException(
            'No linked account found with that account number at your bank',
          );
        }

        const internalReference = this.generateInternalReference('BNK');

        // Received -> Validated -> (Allocated | stays Validated), same
        // progression as TelecomService.recordContribution().
        const [transaction] = await manager.query<BankTransactionRow[]>(
          `INSERT INTO bank_transactions
             (member_bank_account_id, transaction_reference, internal_reference, transaction_type, amount, transaction_status, transaction_date)
           VALUES ($1, $2, $3, 'Contribution', $4, 'Received', NOW())
           RETURNING bank_transaction_id, transaction_reference, internal_reference, transaction_type, amount, transaction_status, transaction_date`,
          [
            account.member_bank_account_id,
        ...
        await manager.query(
          `UPDATE bank_transactions SET transaction_status = 'Validated' WHERE bank_transaction_id = $1`,
        ...
        const { walletTransaction, allocation } =
          await this.walletsService.creditContribution(
            ...
            dto.amount,
            {
              ...
              transactionType: 'Contribution - Bank Transfer',
              transactionReference: dto.transactionReference,
              remarks: `Bank transfer contribution via account ending ${account.account_number.slice(-4)}.`,
            },
          );
        ...
        if (finalStatus === 'Allocated') {
          await manager.query(
            `UPDATE bank_transactions SET transaction_status = 'Allocated' WHERE bank_transaction_id = $1`,
            [transaction.bank_transaction_id],
          );
        }
        ...
        await this.auditLogsService.record(manager, {
          ...
          actionType: 'bank.contribution_record',
          affectedTable: 'bank_transactions',
          affectedRecordId: transaction.bank_transaction_id,
          newValue: {
            amount: dto.amount,
            transactionReference: dto.transactionReference,
            internalReference,
      ...
      if (this.isUniqueViolation(error)) {
        throw new ConflictException(
          'A contribution with this transaction reference has already been recorded.',
        );
      }
      throw error;
    }
  ...
  // Real inbound webhook — authenticated by BankApiKeyGuard against the
  // bank's own api_key_hash. Mirrors TelecomService.
  // handleContributionWebhook() exactly: idempotent on
  // externalTransactionId, computes the contribution via
  // contribution_rules (rule_type='Bank Transfer') rather than trusting
  // a caller-supplied contribution amount, never errors on a
  // redelivered transaction.
  async handleTransactionWebhook(
    bankId: number,
    dto: WebhookTransactionDto,
    ipAddress: string | null = null,
    signatureVerified: boolean = false,
  ) {
    const [existing] = await this.dataSource.query<BankTransactionRow[]>(
      `SELECT bt.bank_transaction_id, bt.transaction_reference, bt.internal_reference, bt.transaction_type, bt.amount, bt.transaction_status, bt.transaction_date
       FROM bank_transactions bt
       JOIN member_bank_accounts mba ON mba.member_bank_account_id = bt.member_bank_account_id
       WHERE bt.transaction_reference = $1 AND mba.bank_id = $2`,
      [dto.externalTransactionId, bankId],
    );

    if (existing) {
      return {
        duplicate: true,
        bankTransactionId: existing.bank_transaction_id,
        amount: existing.amount,
        externalReference: existing.transaction_reference,
    ...
    try {
      return await this.dataSource.transaction(async (manager) => {
        ...
        // Received -> Validated -> (Allocated | stays Validated), same
        // progression as TelecomService.handleContributionWebhook().
        const [transaction] = await manager.query<BankTransactionRow[]>(
          `INSERT INTO bank_transactions
             (member_bank_account_id, transaction_reference, internal_reference, transaction_type, amount, transaction_status, transaction_date)
           VALUES ($1, $2, $3, 'Contribution', $4, 'Received', NOW())
        ...
        const { walletTransaction, allocation } =
          await this.walletsService.creditContribution(
            ...
            contributionAmount,
            {
              ...
              remarks: `Bank transfer contribution via account ending ${account.account_number.slice(-4)} (webhook).`,
            },
          );
        ...
        await this.auditLogsService.record(manager, {
          memberId: account.member_id,
          actionType: 'bank.webhook_transaction',
          affectedTable: 'bank_transactions',
          affectedRecordId: transaction.bank_transaction_id,
          newValue: {
      ...
      if (this.isUniqueViolation(error)) {
        const [raced] = await this.dataSource.query<BankTransactionRow[]>(
          `SELECT bt.bank_transaction_id, bt.transaction_reference, bt.internal_reference, bt.transaction_type, bt.amount, bt.transaction_status, bt.transaction_date
           FROM bank_transactions bt
           JOIN member_bank_accounts mba ON mba.member_bank_account_id = bt.member_bank_account_id
           WHERE bt.transaction_reference = $1 AND mba.bank_id = $2`,
        ...
        return {
          duplicate: true,
          bankTransactionId: raced?.bank_transaction_id,
          amount: raced?.amount,
          externalReference: raced?.transaction_reference,
          internalReference: raced?.internal_reference,
          transactionStatus: raced?.transaction_status,
          transactionDate: raced?.transaction_date,
        };
      }
      throw error;
    }
  ...
  // Bank's mirror of TelecomService.reverseContribution() — only a
  // Confirmed Contribution-type bank_transactions row can be reversed,
  // guarded inside the same transaction that flips its status. Deposit/
  // Withdrawal rows use this same table but go through
  // updateTransactionStatus() instead, not this method.
  async reverseContribution(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      >(
        `SELECT bt.bank_transaction_id, mba.member_id, bt.amount, bt.transaction_reference, bt.transaction_status, bt.transaction_type
         FROM bank_transactions bt
         JOIN member_bank_accounts mba ON mba.member_bank_account_id = bt.member_bank_account_id
         WHERE bt.bank_transaction_id = $1 AND mba.bank_id = $2`,
        [bankTransactionId, bankId],
      );

      if (!transaction) {
        throw new NotFoundException('Transaction not found for your bank');
      }

      if (transaction.transaction_type !== 'Contribution') {
        throw new BadRequestException(
          'Only a Contribution-type transaction can be reversed through this endpoint.',
        );
      }

      if (transaction.transaction_status !== 'Allocated') {
        throw new BadRequestException(
          `Only an Allocated contribution can be reversed (this one is ${transaction.transaction_status}).`,
        );
      }
      ...
      >(
        `SELECT wallet_transaction_id FROM wallet_transactions WHERE bank_transaction_id = $1 LIMIT 1`,
        [bankTransactionId],
      );
      ...
      await this.walletsService.reverseContribution(
        ...
        Number(transaction.amount),
        {
          ...
          remarks: `Reversal of Bank contribution #${bankTransactionId}.`,
          reason: 'Reversed',
        },
      );

      await manager.query(
        `UPDATE bank_transactions SET transaction_status = 'Reversed' WHERE bank_transaction_id = $1`,
        [bankTransactionId],
      );

      await this.auditLogsService.record(manager, {
        memberId: transaction.member_id,
        actionType: 'bank.contribution_reverse',
        affectedTable: 'bank_transactions',
        affectedRecordId: bankTransactionId,
        oldValue: { transactionStatus: 'Allocated' },
        newValue: { transactionStatus: 'Reversed' },
        ipAddress,
      });
  ...
  // against) and is now being voided. Once Allocated, use
  // reverseContribution() instead. Mirrors
  // TelecomService.markContributionFailed() exactly.
  async markContributionFailed(
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      if (transaction.transaction_type !== 'Contribution') {
        throw new BadRequestException(
          'Only a Contribution-type transaction can be marked failed through this endpoint.',
        );
      }

      if (!['Received', 'Validated'].includes(transaction.transaction_status)) {
        throw new BadRequestException(
          `Only a Received or Validated contribution can be marked Failed (this one is ${transaction.transaction_status}). An Allocated contribution must be reversed instead.`,
        );
      }
      ...
      await this.walletsService.reverseContribution(
        ...
        Number(transaction.amount),
        {
          bankTransactionId,
          transactionReference: transaction.transaction_reference,
          remarks: `Bank contribution #${bankTransactionId} marked failed.`,
          reason: 'Failed',
        },
      );

      await manager.query(
        `UPDATE bank_transactions SET transaction_status = 'Failed' WHERE bank_transaction_id = $1`,
        [bankTransactionId],
      );

      await this.auditLogsService.record(manager, {
        memberId: transaction.member_id,
        actionType: 'bank.contribution_fail',
        affectedTable: 'bank_transactions',
        affectedRecordId: bankTransactionId,
        oldValue: { transactionStatus: transaction.transaction_status },
        newValue: { transactionStatus: 'Failed' },
        ipAddress,
      });
    ...
    return this.dataSource.transaction(async (manager) => {
      ...
      const [settlement] = await manager.query<SettlementRow[]>(
        `INSERT INTO settlements
           (bank_id, counterparty_type, counterparty_name, telecom_operator_id, insurance_provider_id, amount, settlement_status, initiated_by)

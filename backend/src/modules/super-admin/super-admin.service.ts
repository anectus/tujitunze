...
export class SuperAdminService {
  ...
  async listAdministrators() {
    const rows = await this.dataSource.query<AdministratorRow[]>(
      `SELECT u.user_id, u.first_name, u.second_name, u.surname, u.email,
              ...
              b.bank_name, t.operator_name, ip.provider_name
       FROM users u
    ...
    return rows.map((row) => ({
      ...
      role: row.role_name,
      tenantName:
        row.bank_name ?? row.operator_name ?? row.provider_name ?? null,
    }));
  }
  ...
  async listTenants(): Promise<{
    ...
    const [banks, telecomOperators, insuranceProviders] = await Promise.all([
    ...
    return { banks, telecomOperators, insuranceProviders };
  }
  ...
  private async countByStatus(
    table: 'banks' | 'telecom_operators' | 'insurance_providers',

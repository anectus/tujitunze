...
// Also covers the forgot/verify/resend/reset password-recovery routes
// (see password-reset.e2e-spec.ts for their functional behavior, which
// overrides ThrottlerGuard so it doesn't collide with these limits).
...
describe('Rate limiting (e2e)', () => {
  ...
  it('throttles POST /auth/forgot-password at 3/min per IP', async () => {
    const statuses: number[] = [];
  ...
  it('throttles POST /auth/verify-reset-otp at 5/min per IP', async () => {
    const statuses: number[] = [];
  ...
  it('throttles POST /auth/resend-reset-otp at 3/min per IP', async () => {
    const statuses: number[] = [];
  ...
  it('throttles POST /auth/reset-password at 5/min per IP', async () => {
    const statuses: number[] = [];

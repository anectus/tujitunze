...
interface InsuranceDashboardResponse {
...
describe('Super-admin administrator provisioning (e2e)', () => {
  ...
  it('rejects an Insurance account with no tenantId', async () => {
  ...
  it('rejects an Insurance account with a nonexistent tenantId', async () => {
    const token = signToken(superAdminUserId, ['Super-admin'], 'SuperAdmin');
  ...
  it('creates an Insurance administrator who can then log in and reach their scoped dashboard', async () => {
    const superAdminToken = signToken(
      superAdminUserId,
      ['Super-admin'],
      'SuperAdmin',
    );
    const email = `new-insurance-staff-${ts}@test.local`;
    ...
    expect(created.role).toBe('Insurance');
    ...
    const dashboard = dashboardResponse.body as InsuranceDashboardResponse;
    expect(dashboard.provider.name).toBe(`E2E Provisioning Insurance ${ts}`);
  });
});
